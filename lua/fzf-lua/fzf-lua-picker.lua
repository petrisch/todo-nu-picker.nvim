...
M.fzf_pick_todo = function(todo_opts)
	...
	local fzf_config = vim.deepcopy(fzf_lua.config or {})
	-- Snacks.notifier.notify("fzf_config: " .. vim.inspect(fzf_config), "warn")

	local fzf_actions_files = vim.deepcopy(fzf_config.defaults.actions.files or {})
	...
	local actions = vim.tbl_deep_extend("force", fzf_actions_files, my_actions)
	...
	Snacks.notifier.notify("config: " .. vim.inspect(config), "warn")

	local opts = vim.tbl_deep_extend("force", fzf_config, config)
